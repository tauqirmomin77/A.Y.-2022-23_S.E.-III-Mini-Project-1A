...
public class TankClient extends Frame implements ActionListener {
	...
	public void framPaint(Graphics g) {
		...
		if(!Player2)g.drawString("Tanks left in the field: ", 200, 100);
		else g.drawString("Tanks left in the field: ", 110, 100);
		g.setFont(new Font("Times New Roman", Font.ROMAN_BASELINE, 30));
		if(!Player2)g.drawString("" + tanks.size(), 400, 100);
		else g.drawString("" + tanks.size(), 300, 100);
		g.setFont(new Font("Times New Roman", Font.BOLD, 20));
		if(!Player2)g.drawString("Health: ", 580, 100);
		else g.drawString("Health: ", 380, 100);
		g.setFont(new Font("Times New Roman", Font.ROMAN_BASELINE, 30));
		if(!Player2) g.drawString("" + homeTank.getLife(), 650, 100);
		else g.drawString("Player1: " + homeTank.getLife()+"    Player2:"+homeTank2.getLife(), 450, 100);
		g.setFont(f1);
		if (!Player2){
